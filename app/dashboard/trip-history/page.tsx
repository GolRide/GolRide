...
export default async function TripHistoryPage() {
      ...
      <main className="bg-slate-50 text-slate-900">
        <div className="mx-auto max-w-6xl px-4 py-8">
          <div className="rounded-2xl border border-slate-200 bg-white p-6">
            ...
            <div className="mt-2 text-slate-600">Entra para ver tu histórico de viajes.</div>
            <Link
              href="/login?next=/dashboard/trip-history"
  ...
  // Histórico = viajes inactivos (active: false).
  // Si luego quieres que sea "viajes ya pasados por fecha", lo cambiamos.
  const trips = await Trip.find({ creatorId: session.userId, active: false })
    .sort({ date: -1 })
    .lean();
    ...
    <main className="bg-slate-50 text-slate-900">
      <div className="mx-auto max-w-6xl px-4 py-8">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-black">Histórico de viajes</h1>
            <p className="mt-1 text-slate-600">Aquí verás tus viajes finalizados o eliminados.</p>
          </div>
        ...
        <div className="mt-5 grid gap-3">
          ...
          {trips.length === 0 ? (
            <div className="rounded-2xl border border-slate-200 bg-white p-6 text-slate-700">
              No tienes viajes en el histórico todavía.
            </div>
          ) : null}
        </div>
      </div>
