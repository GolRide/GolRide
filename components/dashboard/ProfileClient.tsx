...
import ReservedSuccessModal from "@/components/dashboard/ReservedSuccessModal";
...
function IconEye({ open }: { open: boolean }) {
  return open ? <span aria-hidden="true">👁️</span> : <span aria-hidden="true">🙈</span>;
}
...
export default function ProfileClient({ initialUser }: { initialUser: UserDTO }) {
  ...
  // (visual) dejamos esto, aunque no guardemos password aún
        ...
        <div className="rounded-2xl border border-slate-200 bg-sky-50 p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              ...
              onClick={save}
              disabled={saving}
              className="inline-flex items-center justify-center rounded-xl bg-black px-4 py-2 text-sm font-extrabold text-white hover:opacity-90 disabled:opacity-50"
            >
          ...
          <div className="mt-6 grid gap-4">
            <div className="rounded-2xl border border-slate-200 bg-white/80 p-4">
              ...
              <div className="mt-3 flex items-center gap-4">
                ...
                <label className="inline-flex cursor-pointer items-center rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-extrabold text-slate-900 hover:bg-slate-50">
                  Subir foto
                  <input
                    type="file"
                    accept="image/*"
            ...
            <Row label="Email" value={user.email} editable={false} />
